...
if (isset($_GET['id']) && isset($_GET['token'])) {
    $db = App::getDatabase();
    $user = App::getUser()->checkReset($db, $_GET['id'], $_GET['token']);
    if ($user) {
        if (!empty($_POST)) {
            ...
            if ($validator->isValid()) {
                ...
                $db->query('UPDATE users SET password = :pass, reset_at = NULL , reset_token = NULL WHERE id_user = :id',
                    [':pass' => $password, ':id' => $_GET['id']]);
                Session::getInstance()->setFlash('success','Votre mot de passe a bien été modifié');
                App::getUser()->connect($user);
